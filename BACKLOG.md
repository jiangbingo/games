...
**泄露 key 验证（2026-09-03，已闭环）**：从 git 历史提取的旧 BigModel key 实测 API 返回 401 身份验证失败——已失效（吊销或过期），风险闭环，无需控制台操作。注意仓库为公开仓库，历史不可随意重写。
...
### T3-1 贪吃蛇增强（0.5-1 天）✅ 已完成（2026-09-03）

- 触控审计：现有 5 处 touch/pointer 处理对齐 T2-1 范式（手指跟随、意图轴消费、碰壁语义），桌面键盘保留 → **已实施**：touchend 滑动改为 touchmove 手指跟随（0.62 格阈值、主导轴消费、轴守卫禁 180° 掉头）；D-pad 换 pointerdown 降为辅助；点画布开局/重开
- 内容增强（T3-3 锦鲤禅庭视觉参考，儿童可读性优先）：主题化皮肤 ×2、吃食反馈动效（缩放/涟漪，只用 transform/opacity）、速度随得分平滑爬升 → **已实施**：森林/锦鲤双皮肤（CSS 变量全站主题化 + localStorage 记忆）、双环涟漪 + "+10" 飘字（canvas 逐帧）、每食 ×0.98 爬速地板 45%
- 验收：生产站离线可玩、iPad Safari 手指跟随无误触、动画不掉帧 → Playwright E2E 15/15（含 CDP 真触摸）；iPad 真机归入 T1-3/T2-4 清单
- **顺手修复**：START 双触发（内联 onclick + listener）；21 处 [DEBUG] 日志清零；setSpeed 中途重开清分；alert 换画布内结算 overlay；标题/界面中文化

### T3-2 水墨方块增强（1-1.5 天）✅ 已完成（2026-09-03）

- 触控方案 → **已实施**：横向拖动=平移（位移跨事件累积、每满一格消费一步）、慢下滑=软降（净位移去重消费）、快滑≥3 格=速降、点画布=旋转；屏幕 D-pad 保留为辅助；画布 touch-action: none
- 本地化 → **已实施**：Google Fonts（Ma Shan Zheng + Noto Serif SC 400/700）pyftsubset 按页面字符子集化共 ~110KB，自托管 `classic-games/fonts/` 并进 SW 预缓存；`cdn.hailuoai.com` 备案图标本地化 `classic-games/icons/beian-icon.png`；minimax 页脚署名与外链保留（来源合规）。**顺修存量 bug**：页尾署名样式块的 `*` reset 与 `body font-family` 覆盖水墨主题全局字体（700 字重永不加载），已删除全局污染规则；右下中国结 SVG path 存量损坏（每次报 pageerror），已复用左上 path
- 内容增强 → **已实施**：行消除墨晕扩散（每消行一团墨点扩散淡出）+ 连击提示（≥2 连击飘字，断连清零）；难度曲线前 3 级缓坡（1000/950/900ms）后续每级 -100ms 地板 100ms
- 验收：Playwright E2E 15/15（无远程请求/字体加载/拖动/点按/软降/速降/墨晕/连击/断连/难度曲线）；单手完整一局待 iPad 真机（T1-3/T2-4）

### T3-1 + T3-2 发布记录（✅ 2026-09-03 已上生产）

- 提交：`9e5d695`（T3-1）、`4dc8dab`（T3-2 + 字体/图标资产 + sw 预缓存 19→23）、`1db9414`（PWA 修复，见下）
...
## 计划外新增（2026-09-12 完成登记）

| # | 条目 | 验收 |
|---|---|---|
| X1 | ✅ 2026-09-12 完成（用户新增需求，随 ef3babd + 本次推送上线）：**记忆翻牌重写**——物品主题选择（随机/蔬菜/动物/厨房/花草/水果，映射换肤不重开、进度保留）+ 难度递增 7 关（4→10 对，通关循环）+ 2 个真缺陷修复（跨重建定时器软锁、结算窗口吞奖励）；**共享底座 js/kids-ui.js 新增主音量**（顶栏滑杆、持久化 `kids_hub_volume`；音量语义：静音只静音效、TTS/动物叫声等内容音频只随主音量） | 3 轮红队 + 1 轮生态审查 PASS；Playwright E2E 约 130 断言全绿 + 线上实测；登记见 CHANGELOG v4.2 |
